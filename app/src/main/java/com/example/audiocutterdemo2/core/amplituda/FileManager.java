...
import com.example.audiocutterdemo2.core.amplituda.exceptions.io.AmplitudaIOException;
...
final class FileManager {
    ...
    FileManager(final Context context) {
        ...
        cache = cachePath + File.separator;
    }
    ...
    synchronized File getByteArrayFile(InputStream inputStream, final AmplitudaProgressListener listener) {
        File temp = new File(cache, String.valueOf(System.currentTimeMillis()));

        try (FileOutputStream outputStream = new FileOutputStream(temp)) {
            if (listener != null) listener.onProgressInternal("", 0, "", 0);
...
//            if (listener != null) listener.onProgressInternal("", 100, "", 0);
            return temp;
        } catch (IOException ignored) {
            return null;
        }
            ...
            while ((read = inputStream.read(buffer)) != -1) {
                ...
                if (listener != null && contentLength > 0) {
                    ...
                    if (current_progress != progress) {
                        listener.onProgressInternal("", current_progress, "", 0);

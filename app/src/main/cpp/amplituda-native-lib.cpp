...
Java_com_example_audiocutterdemo2_core_amplituda_Amplituda_amplitudesFromAudioJNI(
        JNIEnv *env,
        jobject,
        jstring jaudio_path,
        jint jcompress_type,
    ...
    if (jlistener != NULL) {
        jclass listener_class = env->FindClass(
                "com/example/audiocutterdemo2/core/amplituda/AmplitudaProgressListener");
        on_progress_method = env->GetMethodID(listener_class, "onProgressInternal",
    ...
    jclass amplitudaResultClass = (env)->FindClass(
            "com/example/audiocutterdemo2/core/amplituda/AmplitudaResultJNI");
...
// also prevent division by zero exception
    if (audio_dec_ctx->frame_size > 0) {
        nb_frames = (int) ((audio_dec_ctx->sample_rate * duration) / audio_dec_ctx->frame_size);
    }
...
// read frames from the file
    while (av_read_frame(fmt_ctx, pkt) >= 0 && isActive) {
        ...
        if (is_audio_stream) {
            ...
            if (compress_type != COMPRESS_NONE && current_frame_idx % compression_divider == 0) {
                std::string temp_sample_str = compress_temp_amplitudes_data(&temp_data,
                                                                            &compress_type);
        ...
        if (valid_listener) {
            ...
            if (current_progress != progress && progress <= 99) {
                //ManhNQ - only create when current progress change and clear data after done task avoid Out Of Memory
                jstring result = jaudio_path;
                if (juri != nullptr) {
                    result = juri;
                }

                data = env->functions->NewStringUTF(env, amplitudes_data.c_str());

                env->CallVoidMethod(jlistener, on_progress_method, result, progress, data,
                                    nb_frames);
...
// make last progress listener call
    if (valid_listener && current_progress < 100 && isActive) {
        jstring result = jaudio_path;
        if (juri != nullptr) {
            result = juri;
        }
        env->CallVoidMethod(jlistener, on_progress_method, result, 100, data, nb_frames);
    }
...
Java_com_example_audiocutterdemo2_core_amplituda_Amplituda_setActive(JNIEnv *env, jobject,
                                                                jboolean active) {
    isActive = (int) active;
}